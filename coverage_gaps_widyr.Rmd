...
parr %>% 
  select(pcode3, nutricion_ben:sa_ben) %>% 
  pivot_longer(cols = c(nutricion_ben:sa_ben), names_to = "sector", values_to = "beneficiaries") %>% 
  mutate(sector = str_replace_all(sector, c("_ben" = ""))) %>% 
  select(pcode3, sector, beneficiaries) %>%
  filter(beneficiaries != 0) %>% 
  pairwise_count(sector, pcode3) %>% 
  arrange(desc(n))
...
```{r}
act_ben %>% 
  select(pcode3, sector, beneficiarios) %>% 
  pairwise_cor(sector, pcode3, beneficiarios, upper = FALSE) %>% 
...
act_ben %>% 
  ...
  distinct() %>% 
  pairwise_count(categoria, pcode3) %>% 
  arrange(desc(n)) %>% 
...
act_ben %>% 
  ...
  filter(sector1 != sector2) %>% 
  mutate_at(vars(item1, item2), ~str_to_title(.)) %>% 
  select(actvity_cat1 = item1, activity_cat2 = item2, count = n) %>% 
  head(15) %>% 
  kbl(caption = "Most common inter-cluster activity category combinations") %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
